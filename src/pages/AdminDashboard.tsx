            ...
            >
              {filteredUsers.map((user) => (
                <div key={user.uid} className="neon-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-white/5">
                  ...
                  <div className="w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t border-white/5 sm:border-t-0 flex justify-between sm:justify-end">
                    <p className="text-[8px] text-gray-500 font-bold uppercase tracking-widest leading-relaxed">Member since {new Date(user.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}rt, Users, Search, Filter, CheckCircle, Info, XCircle, ArrowRight, 
  Trash2, Pause, Play, AlertCircle, Calendar, Hash, Tag, User as UserIcon, Store, ShoppingBag, ExternalLink, Loader2,
  X, AlertTriangle
} from 'lucide-react';
import { db, handleFirestoreError, OperationType } from '../lib/firebase';
